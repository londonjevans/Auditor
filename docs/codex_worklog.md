...
REMAINING_ACTIONABLE_TICKETS: Provider-free exact candidate cost admission is implemented in WIP; committed-byte preflight reconciliation remains actionable. Credentialed execution remains `BLOCKED_SAFETY`; external authority-log publication and all benchmark/release work remain queued behind genuine REAL runner evidence.
...
- **Next exact action:** After the root commits the exact-cost bytes, rerun the unchanged
  provider-free r2/r4/r2 preflight on that commit and retain its exact nonauthorizing output. Then
