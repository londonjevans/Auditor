...
FIXTURE = Path("tests/fixtures/solidity/economic_temporary_liquidity_oracle")
...
def test_real_foundry_temporary_liquidity_regression_is_settled_and_replayable(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "TemporaryLiquidityOracle.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()

    first = _run_contract(
        forge,
        "UnsafeTemporaryLiquidityInvariant",
    ...
    safe = _run_contract(
        forge,
        "SafeTemporaryLiquidityInvariant",
        tmp_path / "safe",
    )
    ...
    for control in (
        "testSetupAloneHasNoTemporaryLiquidityTransition",
        "testUnsafeTransitionViolatesNoExcessInvariant",
        "testSafeTransitionSettlesWithoutExcess",
    ):
        result = _run_contract(
            forge,
            "TemporaryLiquiditySettlementControls",
            tmp_path / "controls",
            match_test=control,
        )
