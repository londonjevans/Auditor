...
FIXTURE = Path("tests/fixtures/solidity/economic_upgrade_initializer")
...
def test_real_foundry_upgrade_regressions_use_only_proxy_calls(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "UpgradeInitializer.sol"
    test_source = FIXTURE / "test" / "UpgradeInitializer.t.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()
    test_hash = hashlib.sha256(test_source.read_bytes()).hexdigest()
    test_text = test_source.read_text(encoding="utf-8")
    assert all(token not in test_text for token in ("vm.store", "vm.etch", "deployCode"))

    first = _run_contract(forge, "UnsafeUpgradeInvariant", tmp_path / "first")
    second = _run_contract(forge, "UnsafeUpgradeInvariant", tmp_path / "second")
    ...
    safe = _run_contract(forge, "SafeUpgradeInvariant", tmp_path / "safe")
    assert safe.returncode == 0, safe.stdout + safe.stderr
    assert _normalized(safe) is InvariantExecutionStatus.PASSED

    for control in (
        "testSafeProxyRejectsRepeatInitializationAndUnauthorizedUpgrade",
        "testAuthorizedUpgradeUsesTheProxyEntryPoint",
        "testUnsafeValidTransitionsDoNotRecordFailure",
    ):
        result = _run_contract(
            forge,
            "UpgradeInitializerControls",
            tmp_path / "controls",
            match_test=control,
        )
    ...
    assert hashlib.sha256(test_source.read_bytes()).hexdigest() == test_hash
