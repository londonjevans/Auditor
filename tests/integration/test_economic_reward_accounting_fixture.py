...
FIXTURE = Path("tests/fixtures/solidity/economic_reward_accounting")
...
def test_real_foundry_reward_regressions_are_replayable_and_minimal(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "RewardAccounting.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()

    for contract in ("UnsafeRewardIndexInvariant", "UnsafeClaimOnceInvariant"):
    ...
    for contract in ("SafeRewardIndexInvariant", "SafeClaimOnceInvariant"):
        result = _run_contract(forge, contract, tmp_path / "safe")
        assert result.returncode == 0, result.stdout + result.stderr
        assert _normalized(result) is InvariantExecutionStatus.PASSED

    for control in (
        "testAccrualWithoutResetIsMonotonic",
        "testSingleUnsafeClaimDoesNotExceedEntitlement",
    ):
        result = _run_contract(
            forge,
            "RewardAccountingMinimalityControls",
            tmp_path / "controls",
            match_test=control,
        )
        assert result.returncode == 0, result.stdout + result.stderr
