"""Real offline invariant handoff and replay using trusted Python, not Forge evidence."""
...
_CONTROL = (
    "from pathlib import Path; "
    "assert Path('src/StateOrdering.sol').is_file(); "
    "harness = Path('test/mmaudit_generated/MMAuditInvariant_ManagedLocalState.t.sol'); "
    "assert 'new SafePreparedStateMachine(' in harness.read_text(); "
    "assert 'invariant_PreparedStateIsNotFinalized' in harness.read_text(); "
    "assert not Path('test/mmaudit_generated/toolchain/solc').exists(); "
    f"print({local_invariant_control_output()!r})"
)
...
class _FixedInvariantControl:
    name = "unverified-prepared-local-invariant-control"
    ...
    def wrap(self, command, *, workspace, private_dir, rpc_port):
        assert workspace.is_relative_to(private_dir) and rpc_port == 0
        ...
        assert command == [
            ...
            str(workspace),
            "--match-path",
            "test/mmaudit_generated/MMAuditInvariant_ManagedLocalState.t.sol",
            "--match-contract",
            "MMAuditInvariant_ManagedLocalState",
            "--no-auto-detect",
            "--use",
            str(self.solc),
            "--offline",
...
def test_prepared_tools_reach_real_local_runner_and_replay_without_path_or_rpc(
    ...
    def forbidden(*args, **kwargs):
        pytest.fail("invariant: managed local preparation cannot execute, discover PATH or use RPC")
    ...
    def guarded_popen(command, *args, **kwargs):
        ...
        if command[1] == "-I":
            ...
            assert kwargs["env"]["FOUNDRY_INVARIANT_RUNS"] == "2"
            assert kwargs["env"]["FOUNDRY_INVARIANT_DEPTH"] == "1"
    ...
    if mutation in {"forge", "solc"}:
        ...
        selected.write_bytes(b"Changed inert invariant fixture; never execute.\n")
    ...
    def run():
        return invariant.FoundryInvariantRunner(
            ...
            host_tools=material,
            backend=_FixedInvariantControl(forge, solc, mutation),
        ).run(
    ...
    if mutation != "none":
        ...
        result = run()
        assert result.status is InvariantExecutionStatus.PASSED and result.replay_confirmed
        assert result.execution_evidence is ExecutionEvidenceKind.UNVERIFIED
        assert result.compiler_version == f"Python {version}"
        assert result.compiler_sha256 == result.executable_sha256 == digest
        assert result.attempts == result.successful_attempts == 2
        assert all(item.fresh_workspace for item in result.attempt_evidence)
        assert result.harness_spec_sha256 == specification.specification_sha256()
