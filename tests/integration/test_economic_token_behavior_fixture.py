...
FIXTURE = Path("tests/fixtures/solidity/economic_token_behavior")
...
def test_real_foundry_token_behavior_regressions_are_replayable_and_minimal(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "TokenBehavior.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()

    for contract in ("UnsafeFeeAccountingInvariant", "UnsafeRebaseAccountingInvariant"):
    ...
    for contract in ("SafeFeeAccountingInvariant", "SafeRebaseAccountingInvariant"):
        result = _run_contract(forge, contract, tmp_path / "safe")
        assert result.returncode == 0, result.stdout + result.stderr
        assert _normalized(result) is InvariantExecutionStatus.PASSED

    for control in (
        "testNoFeeHasNoImmediateDivergence",
        "testNoRebaseHasNoImmediateDivergence",
    ):
        result = _run_contract(
            forge,
            "TokenBehaviorMinimalityControls",
            tmp_path / "controls",
            match_test=control,
        )
        assert result.returncode == 0, result.stdout + result.stderr
