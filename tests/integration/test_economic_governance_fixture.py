...
FIXTURE = Path("tests/fixtures/solidity/economic_governance")
...
def test_real_foundry_governance_regressions_are_replayable_and_minimal(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "GovernanceLifecycle.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()

    first = _run_contract(forge, "UnsafeGovernanceInvariant", tmp_path / "first")
    second = _run_contract(forge, "UnsafeGovernanceInvariant", tmp_path / "second")
    ...
    safe = _run_contract(forge, "SafeGovernanceInvariant", tmp_path / "safe")
    assert safe.returncode == 0, safe.stdout + safe.stderr
    assert _normalized(safe) is InvariantExecutionStatus.PASSED

    for control in (
        "testOnlyDeclaredGovernanceRightsReachLifecycle",
        "testInvalidOrderingAndEarlyExecutionAreRejected",
        "testExactDelayAllowsSafeExecution",
        "testCancellationRemainsTerminal",
        "testUnsafeImplementationAcceptsValidDelayedExecution",
    ):
        result = _run_contract(
            forge,
            "GovernanceLifecycleControls",
            tmp_path / "controls",
            match_test=control,
        )
