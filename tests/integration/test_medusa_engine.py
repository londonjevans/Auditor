...
from mmaudit.solidity.formal import FormalRunner, MedusaAdapter
from mmaudit.solidity.reproduction import default_isolation_backend
from tests.unit.test_echidna import FIXTURE, _inputs


def test_real_medusa_translated_fixture_replays_in_hardened_isolation(
    tmp_path: Path,
    config_factory,
) -> None:
    raw_executable = shutil.which("medusa")
    if raw_executable is None:
        pytest.skip("medusa is not installed")
    expected_version = os.environ.get("MMAUDIT_TEST_MEDUSA_VERSION")
    if expected_version is None:
        pytest.skip("MMAUDIT_TEST_MEDUSA_VERSION is not configured")
    ...
    runner = FormalRunner(
        FormalConfig(
            enabled=True,
            timeout_seconds=30,
            medusa_version=expected_version,
