...
FIXTURE = Path("tests/fixtures/solidity/economic_rounding")
...
def test_real_foundry_rounding_regressions_are_replayable_and_minimal(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "RoundingAccounting.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()

    first = _run_contract(forge, "UnsafeRoundingInvariant", tmp_path / "first")
    second = _run_contract(forge, "UnsafeRoundingInvariant", tmp_path / "second")
    ...
    safe = _run_contract(forge, "SafeRoundingInvariant", tmp_path / "safe")
    assert safe.returncode == 0, safe.stdout + safe.stderr
    assert _normalized(safe) is InvariantExecutionStatus.PASSED

    for control in (
        "testExactBoundaryDoesNotCreateValue",
        "testSafeLossIsBoundedBelowOneScaleUnit",
        "testOneUnitIsMinimalValueCreatingBoundary",
    ):
        result = _run_contract(
            forge,
            "RoundingMinimalityControls",
            tmp_path / "controls",
            match_test=control,
        )
