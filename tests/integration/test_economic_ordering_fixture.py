...
FIXTURE = Path("tests/fixtures/solidity/economic_ordering")
...
def test_real_foundry_ordering_regression_is_replayable_bounded_and_minimal(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "OrderedSettlement.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()

    first = _run_contract(forge, "UnsafeOrderingInvariant", tmp_path / "first")
    second = _run_contract(forge, "UnsafeOrderingInvariant", tmp_path / "second")
    ...
    safe = _run_contract(forge, "SafeOrderingInvariant", tmp_path / "safe")
    assert safe.returncode == 0, safe.stdout + safe.stderr
    assert _normalized(safe) is InvariantExecutionStatus.PASSED

    for control in (
        "testStagingAlonePreservesTheBound",
        "testOneReorderIsTheMinimalUnsafeTransition",
        "testSafeSettlementRejectsTheIncorrectTransition",
    ):
        result = _run_contract(
            forge,
            "OrderingMinimalityControls",
            tmp_path / "controls",
            match_test=control,
        )
