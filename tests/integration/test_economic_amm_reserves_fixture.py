...
FIXTURE = Path("tests/fixtures/solidity/economic_amm_reserves")
...
def test_real_foundry_amm_reserve_regression_distinguishes_protected_pricing(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "ReservePricing.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()

    first = _run_contract(
        forge,
        "UnsafeSpotReservePricingInvariant",
    ...
    safe = _run_contract(
        forge,
        "SafeProtectedReservePricingInvariant",
        tmp_path / "safe",
    )
    ...
    for control in (
        "testSetupAloneHasNoReserveMovement",
        "testUnsafeSpotDependenceCreatesExcess",
        "testSafeProtectedPriceSettlesWithoutExcess",
    ):
        result = _run_contract(
            forge,
            "ReservePricingControls",
            tmp_path / "controls",
            match_test=control,
        )
