...
FIXTURE = Path("tests/fixtures/solidity/economic_oracle_guards")
...
def test_real_foundry_oracle_guard_regressions_are_replayable_and_minimal(
    tmp_path: Path,
) -> None:
    ...
    source = FIXTURE / "src" / "OracleGuards.sol"
    source_hash = hashlib.sha256(source.read_bytes()).hexdigest()

    first = _run_contract(forge, "UnsafeOracleGuardInvariant", tmp_path / "first")
    second = _run_contract(forge, "UnsafeOracleGuardInvariant", tmp_path / "second")
    ...
    safe = _run_contract(forge, "SafeOracleGuardInvariant", tmp_path / "safe")
    assert safe.returncode == 0, safe.stdout + safe.stderr
    assert _normalized(safe) is InvariantExecutionStatus.PASSED

    for control in (
        "testGuardedImplementationRejectsEachInvalidBoundary",
        "testGuardedImplementationAcceptsValidPreset",
        "testUnsafeValidPresetDoesNotRecordFailure",
    ):
        result = _run_contract(
            forge,
            "OracleGuardBoundaryControls",
            tmp_path / "controls",
            match_test=control,
        )
