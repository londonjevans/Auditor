"""Reproduction archive preparation is exact input selection, never execution evidence."""
...
def no_execution(monkeypatch):
    def forbidden(*args, **kwargs):
        pytest.fail("invariant: reproduction archive preparation cannot execute or open sockets")
...
def test_reproduction_archive_is_independent_of_primary_scanner_or_matrix(prepared):
    ...
    assert selected.reproduction_source_binding.path == prepared[4].name
    assert selected.source_bindings == (selected.reproduction_source_binding,)
...
def test_reproduction_digest_is_strict_nonzero_sha256(tmp_path, value):
    with pytest.raises(ValidationError):
        ManagedForkArchiveSource(archive_root=tmp_path, reproduction_archive_sha256=value)


@pytest.mark.parametrize("field", ["expected_chain_id", "pinned_block_number"])
@pytest.mark.parametrize("missing", [False, True])
def test_reproduction_archive_requires_exact_final_chain_and_block(prepared, field, missing):
    ...
    with pytest.raises(ManagedForkArchiveError):
        prepare_reproduction_archives(prepared, config=config)


@pytest.mark.parametrize("mode", ["reproduction", "smart_contracts", "acknowledgement"])
def test_reproduction_archive_cannot_enable_disabled_or_unacknowledged_execution(prepared, mode):
    config = prepared[1].config
...
def test_reproduction_archive_drift_invalidates_retained_selection(prepared):
    selected = prepare_reproduction_archives(prepared)
...
def test_primary_and_reproduction_roles_remain_separate_and_share_the_read_ceiling(
    prepared, monkeypatch
):
    config = prepared[1].config
    config.scanners.foundry_fork.enabled = True
    source = prepared[3].model_copy(
        update={"primary_archive_sha256": prepared[3].reproduction_archive_sha256}
    )
    selected = prepare_reproduction_archives(prepared, config=config, source=source)
    assert selected.primary_source_binding == selected.reproduction_source_binding
    assert len(selected.source_bindings) == 2 and selected.state_ids == ()
    assert selected.primary_source_binding is not selected.reproduction_source_binding
    monkeypatch.setattr(
    ...
    with pytest.raises(ManagedForkArchiveError):
        prepare_reproduction_archives(prepared, config=config, source=source)
...
def test_invalid_reproduction_deadlines_refuse_before_listening(prepared, deadline):
    selected = prepare_reproduction_archives(prepared)
    with pytest.raises(ManagedForkArchiveError):
        selected.start_reproduction_attempt(
...
def test_maximum_repetition_policy_uses_independent_full_attempt_lifetimes(prepared):
    ...
    assert selected.reproduction_attempt_lifetime_seconds == 1832
...
def test_reproduction_factory_closes_every_acquired_lease_on_startup_failure(
    ...
    class LifecycleControl:
        def __init__(self, replay, *, lifetime_seconds):
            assert replay.source_binding == selected.reproduction_source_binding
...
def test_reproduction_source_drift_cannot_publish_or_replace_a_setup_receipt(
