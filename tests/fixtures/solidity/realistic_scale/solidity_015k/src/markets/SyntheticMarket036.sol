...
abstract contract SyntheticMarket036 is SyntheticAccounting, ISyntheticScaleMarket {
    bytes32 public constant MARKET_ID = keccak256("mmaudit-synthetic-market-036");
    ...
    function riskWindow04(
        ...
        adjustedAssets = assets + assets * 41 / 10_000;
    }

    function riskWindow05(
        uint256 assets
    ) external pure returns (uint256 adjustedAssets) {
        adjustedAssets = assets + assets * 42 / 10_000;
    }

    function riskWindow06(
        uint256 assets
    ) external pure returns (uint256 adjustedAssets) {
        adjustedAssets = assets + assets * 43 / 10_000;
    }

    function riskWindow07(
        uint256 assets
    ) external pure returns (uint256 adjustedAssets) {
        adjustedAssets = assets + assets * 44 / 10_000;
    }

    function riskWindow08(
        uint256 assets
    ) external pure returns (uint256 adjustedAssets) {
        adjustedAssets = assets + assets * 45 / 10_000;
    }

    function riskWindow09(
        uint256 assets
    ) external pure returns (uint256 adjustedAssets) {
        adjustedAssets = assets + assets * 46 / 10_000;
    }

    function riskWindow10(
        uint256 assets
    ) external pure returns (uint256 adjustedAssets) {
        adjustedAssets = assets + assets * 47 / 10_000;
    }

    function riskWindow11(
        uint256 assets
    ) external pure returns (uint256 adjustedAssets) {
        adjustedAssets = assets + assets * 48 / 10_000;
    }
